...
public class StockageBean extends AbstractStockageBean {
    ...
    public StockageBean() {
        ...
        this.emplacements = new TreeMap<Integer, Emplacement>();
        this.nbEmplacements = new TreeMap<Integer, Integer>();
        this.emplacementsDispo = new TreeMap<Integer, Emplacement>();
        this.nbEmplacementsDispo = new TreeMap<Integer, Integer>();
        ...
        try {
            ...
            while(rs.next()) {
                this.containers.put(rs.getInt("emplacement_id"), new Container(rs.getInt("container_id"), rs.getInt("type_id"), rs.getInt("emplacement_id")));
            }
            rs.close();
        ...
        try {
            ...
            while(rs.next()) {
                this.emplacements.put(rs.getInt("emplacement_id"), new Emplacement(rs.getInt("emplacement_id"), rs.getInt("type_id")));
            }
            rs.close();
        } catch(SQLException e) {
            // Notifier la vue de l'erreur
        ...
        try {
            ...
            while(rs.next()) {
                this.nbEmplacements.put(rs.getInt("type_id"), rs.getInt("nb"));
            }
            rs.close();
        } catch(SQLException e) {
            // Notifier la vue de l'erreur
        ...
        try {
            ...
            while(rs.next()) {
                this.emplacementsDispo.put(rs.getInt("emplacement_id"), new Emplacement(rs.getInt("emplacement_id"), rs.getInt("type_id")));
            }
            rs.close();
        } catch(SQLException e) {
            // Notifier la vue de l'erreur
        ...
        try {
            ...
            // La jointure est là pour exclure les emplacements de type indéfini
            Statement stat = conn.createStatement();
            ResultSet rs = stat.executeQuery("SELECT COUNT(*) AS nb, a.type_id FROM emplacement a NATURAL JOIN type b LEFT JOIN container c ON a.emplacement_id = c.emplacement_id WHERE c.container_id ISNULL GROUP BY a.type_id;");
            
            while(rs.next()) {
                this.nbEmplacementsDispo.put(rs.getInt("type_id"), rs.getInt("nb"));
            }
            rs.close();
        } catch(SQLException e) {
            // Notifier la vue de l'erreur
